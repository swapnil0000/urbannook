...
export default function WhatsAppOneTap() {
  ...
  return (
    <div
      ...
      /* Top right, the same corner Google One Tap uses, but pushed below the
         header so it does not sit on top of it. Stays under the floating chat
         button's z-9999. */
      className="fixed z-[9998] top-20 left-4 right-4 sm:top-24 sm:right-6 sm:left-auto sm:w-80
                 animate-in fade-in slide-in-from-top-4 duration-300"
    >
      <div className="rounded-2xl bg-white shadow-2xl border border-gray-100 p-4 flex items-center gap-3">
        <div className="shrink-0 w-10 h-10 rounded-full bg-[#25D366]/10 flex items-center justify-center">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold text-[#1c3026] leading-tight">
            Sign in with WhatsApp
          </p>
          <p className="text-xs text-gray-500 mt-0.5">
            No password, no OTP to type.
          </p>
        </div>
        ...
        <button
          ...
          onClick={handleClick}
          disabled={isStarting}
          className="shrink-0 rounded-full bg-[#25D366] px-4 py-2 text-xs font-black text-white
                     transition-all hover:bg-[#1da851] disabled:opacity-60"
        >
          {isStarting ? '…' : 'Continue'}
        </button>
        ...
        <button
          ...
          onClick={dismiss}
          aria-label="Dismiss"
          className="shrink-0 w-7 h-7 rounded-full text-gray-300 hover:text-gray-500 transition-colors"
        >
