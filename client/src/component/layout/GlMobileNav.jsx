...
const GlMobileNav = () => {
  ...
  const cell = (active) => `relative flex flex-col items-center gap-1 py-2.5 ${active ? 'text-brand' : 'text-muted'}`;

  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-hair grid grid-cols-4 text-[10px] font-bold font-jakarta" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      {items.map((it) => (
      ...
      <button onClick={() => window.dispatchEvent(new Event('openCartDrawer'))} className="relative flex flex-col items-center gap-1 py-2.5 text-muted">
