  ...
  if (coupons.length === 0) {
    return (
      <div className="bg-white rounded-xl p-8 border border-gray-200 text-center">
        <i className="fa-solid fa-ticket text-brand text-3xl mb-3"></i>
        <p className="text-gray-500 text-sm">No coupons available at the moment</p>
        <p className="text-brand text-xs mt-2 uppercase tracking-wide font-bold">Check back later for exclusive offers!</p>
      </div>
    );
  }
      ...
      {coupons.map((coupon) => {
        ...
        return (
          <div
            ...
            className="bg-white rounded-xl border border-gray-200 overflow-hidden transition-all hover:border-brand/50 shadow-sm"
          >
              <div
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {coupon.discountType === 'PERCENTAGE' && (
                        <span className="bg-ink/10 text-ink text-[10px] font-bold px-2 py-0.5 rounded-full border border-ink/20 uppercase tracking-wide">
                          {coupon.discountValue}% OFF
                        </span>
                      )}
                      {coupon.discountType === 'FIXED' && (
                        <span className="bg-brand/10 text-brand text-[10px] font-bold px-2 py-0.5 rounded-full border border-brand/20 uppercase tracking-wide">
                          ₹{coupon.discountValue} OFF
                        </span>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    disabled={isApplying}
                    className="px-4 py-2 bg-brand text-white rounded-lg font-bold uppercase tracking-wider text-[10px] hover:bg-brandHi transition-all disabled:opacity-50 disabled:cursor-not-allowed shrink-0 shadow-md"
                  >
                    {isApplying ? (
                ...
                <div className="px-4 pb-4 pt-4 border-t border-gray-100 bg-[#f5f7f8]">
                  <div className="space-y-2 text-xs text-gray-500">
                    <div className="flex justify-between border-b border-gray-200 pb-2 last:border-0 last:pb-0">
                      ...
                      <span className="text-ink font-medium capitalize">{coupon.discountType.toLowerCase()}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-200 pb-2 last:border-0 last:pb-0">
                    ...
                    {coupon.maxDiscount > 0 && coupon.discountType === 'PERCENTAGE' && (
                      <div className="flex justify-between border-b border-gray-200 pb-2 last:border-0 last:pb-0">
                        ...
                        <span className="text-ink font-medium">₹{coupon.maxDiscount.toLocaleString()}</span>
                      </div>
                    )}
                    {coupon.minCartValue > 0 && (
                      <div className="flex justify-between border-b border-gray-200 pb-2 last:border-0 last:pb-0">
                        <span className="uppercase tracking-wider font-bold text-[9px]">Min Cart Value</span>
                        <span className="text-ink font-medium">₹{coupon.minCartValue.toLocaleString()}</span>
                      </div>
                    )}
                    {coupon.usageLimit > 0 && (
                      <div className="flex justify-between border-b border-gray-200 pb-2 last:border-0 last:pb-0">
                        <span className="uppercase tracking-wider font-bold text-[9px]">Usage Limit</span>
                        <span className="text-ink font-medium">{coupon.usageLimit} times</span>
                      </div>
                    )}
                    {coupon.expiryDate && (
                      <div className="flex justify-between border-b border-gray-200 pb-2 last:border-0 last:pb-0">
