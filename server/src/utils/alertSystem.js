...
const sendAlert = async (pushMsg) => {
  try {
    ...
    if (!process.env.ZOHO_WEBHOOK_TOKEN || process.env.ZOHO_WEBHOOK_TOKEN === 'PASTE_YOUR_WEBHOOK_URL_HERE') {
      console.warn('[WARN] ⚠️ ZOHO_WEBHOOK_TOKEN not configured - Alert will be logged only');
      ...
      console.log(pushMsg);
      return { success: false, message: 'Webhook not configured - logged locally' };
    }
    
    const webhookUrl = process.env.ZOHO_WEBHOOK_TOKEN;
    ...
    if (error.response) {
      ...
      if (error.response.status === 400) {
        ...
        console.error('[FIX] Please verify your Zoho Cliq webhook configuration:');
        console.error('  - Go to Zoho Cliq > Bots > Your Bot > Edit > Copy webhook URL');
